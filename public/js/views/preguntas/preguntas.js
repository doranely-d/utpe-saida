...
var preguntas = {
    ...
    init: function () {
        ...
        this.tblPreguntas.bootstrapTable({
            url: general.base_url + '/preguntas/listar/?opt=1&tipo='+preguntas.txtTipoSolicitud.val(),
    ...
    cargaFrmPregunta: function (datosIn, contPregunta) {
        ...
        $.ajax({
            type: 'GET',
            url: general.base_url + '/preguntas/modal/?opt=2',
            data: datos,
    ...
    cargaFrmTurnado: function (datosIn) {
        ...
        $.ajax({
            type: 'GET',
            url: general.base_url + '/preguntas/modal/?opt=1',
            data: datos,
            contentType: 'application/html; charset=utf-8',
            beforeSend: function (xhr) {
                //  general.block();
    ...
    cargaFrmComentarios: function (datosIn) {
        ...
        $.ajax({
            type: 'GET',
            url: general.base_url + '/preguntas/modal/?opt=3', //obtenemos modal comentarios
            data: datos,
            contentType: 'application/html; charset=utf-8',
            beforeSend: function (xhr) {
                general.block();
            ...
            success: function (resultado) {
                try {
                    preguntas.modalComentario = bootbox.dialog({
                        title: ' <i class="fa fa-comments-o"></i> Comentarios',
                        onEscape: true,
                        animate: true,
                        size: 'smmall',
                        message: resultado,
                    ...
                    preguntas.modalComentario.on('shown.bs.modal', function () {
                        $('.bootbox-close-button').focus();
                        ...
                        var select = document.getElementById('slDependencias');
                        multi( select, {
                            dropdownParent: preguntas.modalComentario,
                            non_selected_header: 'DEPENDENCIAS',
                            selected_header: 'DEPENDENCIAS SELECCIONADAS',
                            search_placeholder: 'Buscar dependencia ...',
                        });
                        ...
                        preguntas.frmTurnado.formValidation({excluded: [':disabled', ':hidden', ':not(:visible)'], live: 'enabled', locale: 'es_ES'})
                            .on('success.form.fv', function (e) {
                                e.preventDefault();
                                preguntas.guardarTurnado();
                                preguntas.modalComentario.modal('hide');
                            });
                    });
                } catch (e) {
                    setTimeout(function () {
    ...
    cargaDependencias: function () {
        $.ajax({
            type: 'GET',
            url: general.base_url + '/preguntas/buscar/?opt=1',
    ...
    accionesFormatter: function (value, row, index) {
        ...
        return [
            '&nbsp;<a class="btn btn-primary btn-lg btn-xs" role="button" href="javascript:preguntas.cargaFrmPregunta(\'' + $.base64.encode(JSON.stringify(row))+'\','+(index+1)+');" data-toggle="tooltip" data-placement="bottom" title="Información"><i class="glyphicon glyphicon-zoom-in"></i></a>',
            '&nbsp;<a class="btn btn-dropbox btn-lg btn-xs" role="button" href="javascript:preguntas.cargaFrmComentarios(\'' + $.base64.encode(JSON.stringify(row))+'\','+(index+1)+');" data-toggle="tooltip" data-placement="bottom" title="Comentarios"><i class="glyphicon glyphicon-comment"></i></a>',
