...
var solicitudes = {
    ...
    init: function () {
        ...
        //Carga wizard y cambia de etapa
        solicitudes.cargaWizard();

        //carga los checkbos con los titulos
        solicitudes.checkboxAcreditacionIdentidad();
    ...
    validaRadio: function () {
        $('input[type="radio"].radio-arco').iCheck({
        ...
        solicitudes.formularioWizard.wizard().on('actionclicked.fu.wizard', function(e, data) {
            ...
            if (isValidStep === false || isValidStep === null) {
                ...
                if (data.step === 1 && data.direction === 'next') {
                    e.preventDefault();
                    var acreditacionTitular = new Array();
                    var acreditacionResponsable = new Array();
            ...
            $.ajax({
                ...
                data: {
                    ...
                    idDerechoArco: $("#txtIdDerecho").val(),
                    ...
                    try {
                        if (resultado.estatus === 'success') {
                            swal({
                                ...
                                window.location = general.base_url + '/solicitudes/index?opt=0&tipo=ARCO';
                            });
                        }else{
                            general.notify('<strong>Mensaje del Sistema</strong><br/><br/>', resultado.mensaje, resultado.estatus, true);
                        }
            ...
            success: function (resultado) {
                try {
                    ...
                    solicitudes.modalPregunta.on('shown.bs.modal', function () {
                        ...
                        //Funcionalidad
                        solicitudes.frmPregunta.formValidation({excluded:[':disabled',':hidden',':not(:visible)'], live: 'enabled', locale: 'es_ES'}).on('success.form.fv', function (e) {
                            e.preventDefault();
